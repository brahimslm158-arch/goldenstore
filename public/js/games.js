  ...
  S.ready((user) => {
    ...
    async function renderForYou() {
      ...
      if (!recentApps.length && !popularApps.length) {
        content.append(S.emptyState('لا توجد ألعاب بعد', 'ميّز تطبيقاً كـ«لعبة» من لوحة التحكم لتظهر هنا.', 'gamepad'));
        return;
      }
      ...
      content.append(posterSection('ألعاب موصى بها', recommended));
    ...
    async function renderTop() {
      ...
      content.innerHTML = '';
      if (!apps.length) { content.append(S.emptyState('لا توجد ألعاب بعد', null, 'gamepad')); return; }
    ...
    async function renderTopRated() {
      ...
      content.innerHTML = '';
      if (!apps.length) { content.append(S.emptyState('لا توجد ألعاب مقيّمة بعد', 'قيّم الألعاب لتظهر هنا الأعلى تقييماً.', 'gamepad')); return; }
      content.append(rankedList(apps));
    }
  });
